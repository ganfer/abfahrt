...
test('preview workflow updates protected main through a pull request', () => {
  ...
  assert.match(workflow, /BRANCH="automation\/readme-previews"/);
  assert.match(workflow, /gh pr create/);
  assert.doesNotMatch(workflow, /^\s*git push\s*$/m);
});

test('README and screenshot workflow keep the preview gallery contract', () => {
  const readme = read('README.md');
  const workflow = read('.github/workflows/widget-screenshot.yml');
  ...
  for (const file of [
    ...
    assert.ok(readme.includes(file), file + ' must be referenced by README');
    assert.ok(workflow.includes(file), file + ' must be generated by the preview workflow');
  }

  assert.match(readme, /Widget klein/);
  assert.match(readme, /Widget groß/);
  assert.match(readme, /Fullscreen · nach Gleis sortiert/);
  assert.match(readme, /<a href="docs\/assets\/widget-preview-small\.png">/);
  assert.match(readme, /<a href="docs\/assets\/widget-preview-large\.png">/);
  assert.match(readme, /<a href="docs\/assets\/fullscreen-preview-platform\.png">/);

  assert.match(workflow, /pull_request:/);
  assert.match(workflow, /push:/);
  assert.match(workflow, /browser-actions\/setup-chrome@v2/);
  assert.match(workflow, /--variant widget-small/);
  assert.match(workflow, /--variant widget-large/);
  assert.match(workflow, /--variant fullscreen-platform/);
  assert.match(workflow, /git status --porcelain/);
  assert.match(workflow, /git rm docs\/assets\/widget-preview\.png/);

  const triggerSection = workflow.split('\npermissions:')[0];
  assert.doesNotMatch(triggerSection, /docs\/assets\/widget-preview-small\.png/, 'generated previews must not retrigger their own workflow');
  assert.doesNotMatch(triggerSection, /docs\/assets\/widget-preview-large\.png/, 'generated previews must not retrigger their own workflow');
  assert.doesNotMatch(triggerSection, /docs\/assets\/fullscreen-preview-platform\.png/, 'generated previews must not retrigger their own workflow');

  assert.match(renderer, /extractObject\('DEFAULT_WIDGET_CONFIG'\)/);
  assert.match(renderer, /extractObject\('DEFAULT_FULLSCREEN_CONFIG'\)/);
  assert.match(renderer, /renderWidget\('small'\)/);
  assert.match(renderer, /renderWidget\('large'\)/);
  assert.match(renderer, /renderFullscreenPlatform\(\)/);
  assert.match(renderer, /fullscreenGroupLabel/);
  assert.match(renderer, /Gleis \$\{platform\}/);
  assert.match(renderer, /Ohne Gleisangabe/);
